...
namespace
  {
      ...
      mblock_t s_transform(mblock_t i_data)
        {
        mblock_t result = 0;
        for(size_t i = 0; i < 4; i++)
          {
          uint8_t byte = get_byte_32(i_data, i);
          byte = _s_table[byte];

          result |= ((mblock_t) byte) << (i * 8);
          }
...
mblock_t perform_round(const mblock_t& i_left_block, const mblock_t& i_right_block, const mblock_t& i_key)
  {
  mblock_t result = s_transform(i_key ^ i_right_block);
  result = rotl32(result, 13);
  return result ^ i_left_block;
  }
